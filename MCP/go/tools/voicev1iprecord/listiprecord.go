...
func ListiprecordHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/IpRecords%s", cfg.BaseURL, queryString)
...
func CreateListiprecordTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_IpRecords",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListiprecordHandler(cfg),
	}
}
