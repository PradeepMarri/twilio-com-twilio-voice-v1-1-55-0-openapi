...
func ListbyoctrunkHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/ByocTrunks%s", cfg.BaseURL, queryString)
...
func CreateListbyoctrunkTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_ByocTrunks",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListbyoctrunkHandler(cfg),
	}
}
